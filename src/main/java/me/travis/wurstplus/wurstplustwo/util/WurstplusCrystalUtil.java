...
public class WurstplusCrystalUtil
{
    ...
    public static List<BlockPos> possiblePlacePositions(final float placeRange, final boolean thirteen) {
        ...
        positions.addAll((Collection)getSphere(getPlayerPos((EntityPlayer)WurstplusCrystalUtil.mc.player), placeRange, (int)placeRange, false, true, 0).stream().filter(pos -> canPlaceCrystal(pos, thirteen)).collect((Collector<? super Object, ?, List<? super Object>>)Collectors.toList()));
